*** Settings ***
Resource        ../resource/items.resource
...
Cenário 4 - Atualizar as informações de um deposito (PATCH)
    Given that a PATCH request is made on the item
...
Cenário 5 - Atualizar as informações de deposito (PUT)
    Given that a PUT request is made on the item
...
Cemário 6 - Deletar o cadastro de um deposito (DELETE)
    Given that a delete request is made on the item
